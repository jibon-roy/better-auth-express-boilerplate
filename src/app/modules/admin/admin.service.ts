...
const updateAdmin = async (id: string, payload: IUpdateAdminPayload) => {
  ...
  if (!isAdminExist) {
    throw new ApiError(StatusCodes.NOT_FOUND, 'Admin Or Super Admin not found');
  }
...
const changeUserRole = async (
  ...
  // 3. Role of Customer  user cannot be changed by anyone. If needed, they have to be deleted and recreated with new role.
  ...
  if (userToChangeRole.role === Role.USER) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      'You cannot change the role of customer. If you want to change the role of customer, you have to delete the user and recreate with new role'
    );
  }
